...
```{r libraryAndDataLoad,  echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
library(caret)
...
nsv
# scale the data in the training and testing datasets
trainingAll[,-53] <- scale(trainingAll[,-53])
testingAll[,-53] <- scale(testingAll[,-53])
...
rfFit <- train(classe ~., method="rf", prox=TRUE, data=training2)
...
```
We use random forest on the second training set (training2) and see that this gives us a `r 100*cmRF$overall[1]`% overall accuracy. Clearly this is much better than the tree method.

```{r boost, echo=FALSE, message=FALSE,warning=FALSE,cache=TRUE}
boostFit <- train(classe ~., method="gbm", data=training3, verbose=FALSE)
...
```
Boosting on training set 3 (training3) gives us a  `r 100*cmBoost$overall[1]`% overall accuracy.
...
```
Lastly, we try linear discrimant analysis on training set 4 (training4) and get a `r 100*cmLDA$overall[1]`% overall accuracy.
...
```{r overall, include=FALSE, message=FALSE,warning=FALSE,cache=TRUE}
#rfAFit <- train(classe ~., method="rf", prox=TRUE, data=training)
#rfAPredict <- predict(rfAFit,newdata=testing)
#cmRFA <- confusionMatrix(testing$classe,rfAPredict)
#cmRFA$overall[1]  # print out overall accuracy
#table(rfAPredict,testing$classe)
#boostAFit <- train(classe ~., method="gbm", data=training, verbose=FALSE)
#boostAPredict <- predict(boosAtFit,newdata=testing)
#cmABoost <- confusionMatrix(testing$classe,boostAPredict)
#cmABoost$overall[1]  # print out overall accuracy
#table(boostAPredict,testing$classe)
```

Based on the above, we estimate our out-of-sample error to be 1 - the accuracy of the random forest method or `r  1-cmRF$overall[1]`. 
